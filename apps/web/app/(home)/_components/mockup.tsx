...
const Mockup = React.forwardRef<HTMLDivElement, MockupProps>(
  ({ className = "", type = "responsive", ...props }, ref) => {
    const typeClass =
      type === "mobile"
        ? "rounded-[48px] max-w-[350px]"
        : "rounded-md";

    return (
      <div
        ref={ref}
        className={`flex relative z-10 overflow-hidden shadow-2xl border border-border/5 border-t-border/15 ${typeClass} ${className}`}
        {...props}
      />
    );
  }
...
const MockupFrame = React.forwardRef<HTMLDivElement, MockupFrameProps>(
  ({ className = "", size = "small", ...props }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={`bg-accent/5 flex relative z-10 overflow-hidden rounded-2xl ${sizeClass} ${className}`}
        {...props}
      />
    );
  }
