...
const Toolbar = ({ selectedTool, setSelectedTool }: { selectedTool: SelectedToolType, setSelectedTool: Dispatch<SetStateAction<SelectedToolType>> }) => {
  ...
  return (
    <div className="flex items-center gap-1 sm:gap-2 md:gap-3 mx-auto w-fit p-1 sm:p-1.5 px-1.5 sm:px-3 md:px-5 rounded-lg md:rounded-xl bg-white fixed inset-x-0 top-4 sm:top-6">
      {tools.map((tool) => {
        return (
          <div key={tool.type} onClick={() => setSelectedTool(tool.type)} className={`relative rounded-lg p-1.5 sm:p-2 md:p-3 hover:bg-black/10 group transition-colors duration-300 hover:scale-110 hover:cursor-pointer ${selectedTool === tool.type ? "bg-blue-100 text-blue-800 hover:bg-blue-100" : "hover:bg-black/10"}`}>
            <tool.icon className="size-3 sm:size-4 md:size-5" strokeWidth={1.5} />
            <span className={`hidden md:block absolute bottom-0.5 right-[3px] text-[10px] font-medium transition-colors group-hover:text-black ${selectedTool === tool.type ? 'text-black' : 'text-muted-foreground'}`}>
              {tool.shortcut}
            </span>
